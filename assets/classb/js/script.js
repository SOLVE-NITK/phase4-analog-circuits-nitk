...
// ctx.drawImage(image, 0, 0, 350, 250, 4, 5, 100, 25);
// // });
// ctx.beginPath();
// ctx.moveTo(100, 100);
// ctx.lineTo(400, 400);
...
function insert() {
  if (num < 10) {
    ...
    newValueCell4.textContent = Pin.toFixed(8);
    newValueCell5.textContent = Pout.toFixed(8);
...
function simulatee() {
  ...
  $("#frequencySpinner").spinner("disable");
  $("#frequencySlider").slider("disable");
  var gr = document.getElementById("graphbutton");
  ...
  // document.getElementById("comments").style.display="show";
...
function generateGraph(velocity, frequency) {
  ...
  for (var i = 1; i <= (frequency * 2) / 1000; i++) {
    var xPos =
      (frequency * 10) / 1000 +
      (i / ((frequency * 2) / 1000)) * (graphCanvas.width - 50);
  ...
  for (var t = 0; t < graphCanvas.width + 100; t++) {
    var y =
      -1 *
        Math.sin(
          (2 * Math.PI * (frequency / 1000) * t) / (graphCanvas.width - 100)
        ) *
        ((graphCanvas.height + 200) / 8) +
...
function generateGraph1(frequency) {
  ...
  graphctx1.fillText(
    "Time (s)->",
    graphCanvas1.width - 70,
    graphCanvas1.height - 75
  );
  graphctx1.closePath();
  ...
  for (var t = 0; t < graphCanvas1.width + 100; t++) {
    var y =
      -1 *
        Math.sin(
          (2 * Math.PI * (frequency / 1000) * t) / (graphCanvas1.width - 100)
        ) *
        ((graphCanvas1.height + 200) / 8) +
      graphCanvas1.height / 2;
...
function plotgraph() {
  ...
  $("#message").text("Vary the voltage value and click on the tabulate button");
}
